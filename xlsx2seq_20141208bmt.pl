...
my $count = $importer1->each(sub {
	...
	#Prüft ob in der entsprechenden Excel-Zeile wirklich Daten vorhanden sind
  	if (($hash{'data'} ne 'n') and (defined $hash{'sys'})) { 
...
$count = $importer2->each(sub {
	...
	#Prüft ob in der entsprechenden Excel-Zeile wirklich Daten vorhanden sind, sonst wird die Verarbeitung abgebrochen
  	if (($hash{'data'} ne 'n') and (defined $hash{'sys'})) { 
		...
		if (not (length $mattype == 1)){$mattype = '-'};
		...
		if (not (length $language == 3)){$language = 'und'}; 
		...
		$language_long = substr($language_long, 0, -2);
		...
		if (not (length $country == 3)) {$country = 'xx-'};
		...
		if (not defined $hash{'260c'}){$hash{'260c'} = $hash{'593a1'}};
		...
		my $ind906;
   		if ((substr ($hash{'906'}, 0,5) eq "Hochs") or (substr ($hash{'906'}, 0,5) eq "Brief") or (substr ($hash{'906'}, 0,5) eq "Geset")){$ind906 = "a"};
		...
		my $ind907;
   		if ((substr ($hash{'907'}, 0,5) eq "Hochs") or (substr ($hash{'907'}, 0,5) eq "Brief") or (substr ($hash{'907'}, 0,5) eq "Geset")){$ind907 = "a"};
   		...
   		if (substr ($hash{'907'}, 0,3) eq "MM "){$ind907 = "j"};
                ...
                my $ind9071;
   		if ((substr ($hash{'9071'}, 0,5) eq "Hochs") or (substr ($hash{'9071'}, 0,5) eq "Brief") or (substr ($hash{'9071'}, 0,5) eq "Geset")){$ind9071 = "a"};
		...
		my $ind9072;
   		if ((substr ($hash{'9072'}, 0,5) eq "Hochs") or (substr ($hash{'9072'}, 0,5) eq "Brief") or (substr ($hash{'9072'}, 0,5) eq "Geset")){$ind9072 = "a"};
		...
		my $signatur = 	$hash{'852d1'} . ' ' . $hash{'852d2'};
		...
		if (not defined $sysnum{$hash{'490w'}}) {$sysnum{$hash{'490w'}} = sprintf("%-9.9d", $hash{'490w'})};
		...
		#Generiert Data Hash
			...
			_id => $sysnum{$hash{'sys'}},
				...
				[593,' ',' ','a',$hash{'593a1'}],
        			[852,' ',' ','a',$hash{'852a'},'b',$hash{'852b'},'c',$hash{'852c'},'d',$signatur,'e',$hash{'852e'}, 'z',$hash{'852z'}, 'x',$hash{'852x'}],
				[856,' ',' ','u',$hash{'856u'},'z',$hash{'856z'}],
				[906,' ',' ',$ind906,$hash{'906'}],
				[907,' ',' ',$ind907,$hash{'907'}],
