...
die "Argumente: $0 Input-Dokument (xlsx), Output-Dokument (MARCXML), Systemnummerbeginn, \n" unless @ARGV == 3;
...
my $exporter = Catmandu::Exporter::MARC->new(file => $ARGV[1], type => 'XML', encoding => 'UTF-8', skip_empty_subfields => 1, pretty => 1);
	...
	#Anpassung an Alma-Format: Feld 035 wird mit dummy-HAN-Systemnummer gefüllt, Rest der MARC-Felder müssen noch angepasst werden
		...
		#_id => $sysnum{$hash{'sys'}},
       	    record => [
                ...
                ['355','0',' ','a',$hash{'3550a1'}],
                ['355','0',' ','a',$hash{'3550a2'}],
                ['490',' ',' ','a',$hash{'490a'},'i',$hash{'490i'},'v',$hash{'490v'},'w', '(HAN)' . $sysnum{$hash{'490w'}} . 'DSV05'],
                ['500',' ',' ','a',$hash{'500a'}],
                ['500',' ',' ','a',$hash{'500a1'}],
                ['500',' ',' ','a',$hash{'500a2'}],
                ['525',' ',' ','a',$hash{'525a'}],
